...
page_header(
    "Non-IT What-If Operational Simulation",
    "Simulate operational perturbations (timeline delays, budget overruns, resource constraints) via backend API endpoints."
)

project = st.session_state.get("selected_project")
if not project:
    st.warning("Please upload a business project first.")
...
st.subheader("Operational Simulation Parameters")
...
with col_s2:
    budget_change = st.slider("Budget Variance (%)", -30, 50, 10)
with col_s3:
    team_reduction = st.slider("Resource Constraint (%)", 0, 50, 10)
...
if st.button("Run What-If Simulation", type="primary", use_container_width=True):
    baseline_score = float(project.get("risk_score", 45.0))
    ...
    with col_metrics:
        ...
        if simulated_risk_score >= 70:
            st.error("Critical Operational Risk Scenario!")
        elif simulated_risk_score >= 45:
            st.warning("Moderate Operational Exposure.")
        else:
            st.success("Manageable Strategic Risk Limits.")
